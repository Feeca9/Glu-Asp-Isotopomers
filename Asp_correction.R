...
asp_cor<- read_excel("Input Matrices Glu Asp.xlsx", 
                     sheet = "AspMatrixNaturAbundanCorrection", range = "B2:Q53", 
                     col_names = FALSE)
...
for(i in 1:ncol(rawdata)) {
  Asp <- rawdata[,c(i)] 
  calasp.iso <-coef(glmnet(asp_cor,Asp, lambda = 0, lower.limits = 0, intercept = FALSE,thresh = 1e-31,upper.limits = 1))[-1]
